...
if not exist "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\clawshop-startup.vbs" (
    copy /Y D:\clawshop\scripts\startup.vbs "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\clawshop-startup.vbs" >nul
    echo   ✓ 启动文件夹快捷方式已添加
) else (
    echo   - 启动文件夹快捷方式已存在
)
