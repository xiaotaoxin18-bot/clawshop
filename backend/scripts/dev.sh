...
# 关键环境变量（确保有兜底值）
export SUDA_DATABASE_URL="${SUDA_DATABASE_URL:-postgresql://appuser:app123456@localhost:5432/inventory_db?schema=workspace_aadkeahc42wbs}"
export FORCE_AUTHN_INNERAPI_DOMAIN="${FORCE_AUTHN_INNERAPI_DOMAIN:-localhost:3000}"
export NODE_ENV="${NODE_ENV:-development}"

# PostgreSQL 路径（从项目相对位置获取）
PGHOME="$(cd "$PROJECT_DIR/../data/pgsql/pgsql" && pwd)"
export PATH="$PGHOME/bin:$PATH"

echo "========================================"
echo "  clawshop - 开发环境启动"
echo "========================================"
echo "SUDA_DATABASE_URL: $SUDA_DATABASE_URL"
echo "SERVER_PORT: ${SERVER_PORT:-3000}"
echo "CLIENT_DEV_PORT: ${CLIENT_DEV_PORT:-8080}"
echo "========================================"
echo ""
...
if pg_isready -q 2>/dev/null; then
  ...
  PGDATA="$(cd "$PROJECT_DIR/../data/pgdata" && pwd)"
  pg_ctl -D "$PGDATA" -l "$PGDATA/logfile" start
  sleep 3
  if pg_isready -q 2>/dev/null; then
    echo "  ✅ PostgreSQL 启动成功"
  else
    echo "  ❌ PostgreSQL 启动失败，请检查 $PGDATA/logfile"
    exit 1
  fi
fi
