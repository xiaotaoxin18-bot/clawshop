...
│
├── data\                  # 数据库文件（本地，不提交至 Git）
│   ├── pgdata\            # PostgreSQL 数据目录
│   └── pgsql\             # PostgreSQL 程序
│
├── .env                   # 环境变量（本地配置，不提交至 Git）
├── CLAUDE.md              # 项目开发文档
└── README.md
```
...
npm run build:client      # 构建前端（Rspack）
...
| 任务 | 触发 | 动作 |
|------|------|------|
| 服务自启 | 用户登录后 30s | 启动 PostgreSQL + 后端 + ngrok |
| 早间采集 | 每天 09:00 | 采集抖店 → 推后端 → 飞书 |
| 晚间采集 | 每天 21:00 | 同上（第二次采集） |
